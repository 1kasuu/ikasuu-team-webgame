...
/* ---------- GAMEPLAY COLLISION ENGINE ----------
   Collision is data-driven. Visual DOM/CSS is only the renderer.
   Official stage objects use explicit collider shapes from JSON; user HTML
   is converted into a solid collider from its layout box + CSS rotation.
*/
...
function angleRadians(deg=0){return Number(deg||0)*Math.PI/180}
function normalizeCollider(o){
  const c=o?.collider||{};
  if(o?.type==='solid')return {kind:'solid',shape:c.shape||'box',angle:Number(o.angle||0),hazard:false};
  if(o?.type==='hazard'){
    const shape=c.shape||(o.kind==='spike'?'triangle':o.kind==='diamond'?'diamond':'box');
    return {kind:'hazard',shape,angle:Number(o.angle||0),inset:Number(c.inset||0)};
  }
  return null;
}
function boxPolygon(x,y,w,h,angleDeg=0){
  const p=[{x,y},{x:x+w,y},{x:x+w,y:y+h},{x,y:y+h}];
  return angleDeg?rotatePoly(p,{x:x+w/2,y:y+h/2},angleRadians(angleDeg)):p;
}
function trianglePolygon(x,y,w,h,dir='up',inset=0,angleDeg=0){
  const i=Math.max(0,Math.min(Math.min(w,h)/2,inset));
  let p;
  if(dir==='down')p=[{x:x+i,y:y+i},{x:x+w-i,y:y+i},{x:x+w/2,y:y+h-i}];
  else if(dir==='left')p=[{x:x+w-i,y:y+i},{x:x+w-i,y:y+h-i},{x:x+i,y:y+h/2}];
  else if(dir==='right')p=[{x:x+i,y:y+i},{x:x+w-i,y:y+h/2},{x:x+i,y:y+h-i}];
  else p=[{x:x+w/2,y:y+i},{x:x+w-i,y:y+h-i},{x:x+i,y:y+h-i}];
  return angleDeg?rotatePoly(p,{x:x+w/2,y:y+h/2},angleRadians(angleDeg)):p;
}
function diamondPolygon(x,y,w,h,inset=0,angleDeg=0){
  const i=Math.max(0,Math.min(Math.min(w,h)/2,inset));
  const p=[{x:x+i,y:y+h/2},{x:x+w/2,y:y+i},{x:x+w-i,y:y+h/2},{x:x+w/2,y:y+h-i}];
  return angleDeg?rotatePoly(p,{x:x+w/2,y:y+h/2},angleRadians(angleDeg)):p;
}
function localLayoutBox(el){
  let x=0,y=0,node=el;
  while(node&&node!==arena){x+=node.offsetLeft||0;y+=node.offsetTop||0;node=node.offsetParent}
  const w=el.offsetWidth,h=el.offsetHeight;
  return {x,y,w,h};
}
function domRotationScale(el){
  const t=getComputedStyle(el).transform;if(!t||t==='none')return {angle:0,sx:1,sy:1};
  try{const m=new DOMMatrixReadOnly(t);return{angle:Math.atan2(m.b,m.a),sx:Math.hypot(m.a,m.b)||1,sy:Math.hypot(m.c,m.d)||1}}
  catch{return{angle:0,sx:1,sy:1}}
}
function domSolidPolygon(el){
  const b=localLayoutBox(el),{angle,sx,sy}=domRotationScale(el);
  const w=b.w*sx,h=b.h*sy,c={x:b.x+b.w/2,y:b.y+b.h/2};
  const p=[{x:c.x-w/2,y:c.y-h/2},{x:c.x+w/2,y:c.y-h/2},{x:c.x+w/2,y:c.y+h/2},{x:c.x-w/2,y:c.y+h/2}];
  return angle?rotatePoly(p,c,angle):p;
}
function getStageObjectElements(){return [...arena.querySelectorAll('.stage-obj')].map(el=>{try{return{el,obj:JSON.parse(el.dataset.object||'{}')}}catch{return{el,obj:{}}}})}
function stageCollider(entry){
  const {el,obj}=entry,x=Number(obj.x||0),y=Number(obj.y||0),w=Number(obj.w||100),h=Number(obj.h||20),c=normalizeCollider(obj);
  if(!c)return null;
  if(c.kind==='solid')return{el,obj,shape:c.shape,poly:boxPolygon(x,y,w,h,c.angle),angle:c.angle,hazard:false,solid:true};
  if(c.shape==='triangle')return{el,obj,shape:'triangle',poly:trianglePolygon(x,y,w,h,obj.dir||'up',c.inset,c.angle),hazard:true,solid:false};
  if(c.shape==='diamond')return{el,obj,shape:'diamond',poly:diamondPolygon(x,y,w,h,c.inset,c.angle),hazard:true,solid:false};
  return{el,obj,shape:'box',poly:boxPolygon(x,y,w,h,c.angle),hazard:true,solid:false};
}
function userSolidColliders(){
  const out=[];
  userEls.querySelectorAll('*').forEach(el=>{
    if(el.closest('#hud'))return;
    const cs=getComputedStyle(el),b=localLayoutBox(el);
    if(cs.position!=='absolute'||b.w<=0||b.h<=0)return;
    if(el.classList.contains('spike')){
      const {angle}=domRotationScale(el),dir=['up','down','left','right'].find(d=>el.classList.contains(d))||'up';
      out.push({el,shape:'triangle',poly:trianglePolygon(b.x,b.y,b.w,b.h,dir,Number(el.dataset.inset||2),angle*180/Math.PI),hazard:true,solid:false});
    }else if(el.classList.contains('diamond-hazard')||el.classList.contains('diamond')){
      const {angle}=domRotationScale(el);out.push({el,shape:'diamond',poly:diamondPolygon(b.x,b.y,b.w,b.h,2,angle*180/Math.PI),hazard:true,solid:false});
    }else if(el.classList.contains('lava')){
      const {angle}=domRotationScale(el);out.push({el,shape:'box',poly:boxPolygon(b.x,b.y,b.w,b.h,angle*180/Math.PI),hazard:true,solid:false});
    }else{
      out.push({el,shape:'box',poly:domSolidPolygon(el),hazard:false,solid:true});
    }
  });
  return out;
}
function getColliders(){return[...getStageObjectElements().map(stageCollider).filter(Boolean),...userSolidColliders()]}
function overlapHorizontal(r,s){return r.right>s.left&&r.left<s.right}
function overlapVertical(r,s){return r.bottom>s.top&&r.top<s.bottom}
...
function resolveAxisSolid(prevRect,nextRect,solidPoly){
  const s=axisAlignedRectFromPoly(solidPoly),movingDown=nextRect.bottom>prevRect.bottom,movingUp=nextRect.top<prevRect.top,movingRight=nextRect.right>prevRect.right,movingLeft=nextRect.left<prevRect.left;
  if(movingDown&&prevRect.bottom<=s.top+1e-4&&nextRect.bottom>=s.top&&overlapHorizontal(nextRect,s)){player.y=s.top-PLAYER_HITBOX.y-PLAYER_HITBOX.h;player.vy=0;player.onGround=true;player.rotation=Math.round(player.rotation/90)*90;playerEl.classList.remove('jumping');return 'top'}
  if(movingUp&&prevRect.top>=s.bottom-1e-4&&nextRect.top<=s.bottom&&overlapHorizontal(nextRect,s)){player.y=s.bottom-PLAYER_HITBOX.y;player.vy=0;return 'bottom'}
  if(movingRight&&prevRect.right<=s.left+1e-4&&nextRect.right>=s.left&&overlapVertical(nextRect,s)){player.x=s.left-PLAYER_HITBOX.x-PLAYER_HITBOX.w;player.vx=0;return 'left'}
  if(movingLeft&&prevRect.left>=s.right-1e-4&&nextRect.left<=s.right&&overlapVertical(nextRect,s)){player.x=s.right-PLAYER_HITBOX.x;player.vx=0;return 'right'}
  return null;
}
function resolveRotatedSolid(hit){
  ...
  if(Math.abs(hit.axis.y)>=Math.abs(hit.axis.x)){
    if(hit.axis.y<0){player.onGround=true;player.vy=0;player.rotation=Math.round(player.rotation/90)*90;playerEl.classList.remove('jumping')}
    else if(player.vy<0)player.vy=0;
  }else player.vx=0;
}
function checkHazards(playerP,colliders){for(const c of colliders){if(!c.hazard)continue;if(sat(playerP,c.poly)){killPlayer();return true}}return false}
function checkGoal(){const g=currentStage?.goal||{x:640,y:80};const r={left:g.x,top:g.y,right:g.x+36,bottom:g.y+54};if(rectsOverlap(getPlayerRect(),r))showWin()}
function updatePhysics(){
  if(player.dead||inputMode!=='play')return;
  const prevRect=getPlayerRect();let moving=false;
  if(keys.ArrowLeft||keys.a){player.vx=-MOVE_SPD;moving=true}
  if(keys.ArrowRight||keys.d){player.vx=MOVE_SPD;moving=true}
  ...
  if(player.x+PLAYER_W>ARENA_W){player.x=ARENA_W-PLAYER_W;player.vx=0}
  if(player.y<0){player.y=0;player.vy=0}
  player.onGround=false;
  const colliders=getColliders();
  for(const c of colliders){
    if(!c.solid||c.hazard)continue;
    if(c.shape==='box'&&Math.abs(c.obj?.angle||0)<0.001){const resolved=resolveAxisSolid(prevRect,getPlayerRect(),c.poly);if(resolved)continue}
    const hit=sat(playerPoly(),c.poly);if(hit)resolveRotatedSolid(hit);
  }
